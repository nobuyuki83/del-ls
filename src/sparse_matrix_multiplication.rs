...
fn symbolic_multiplication(
    ...
    let mut c_row2ind = vec!(0_usize; nrow_a + 1);
    ...
    // ---------
    for irow_a in 0..nrow_a {
        c_row2ind[irow_a + 1] += c_row2ind[irow_a];
    }
    let mut c_ind2col = vec!(0_usize; c_row2ind[nrow_a]);
    // ---------
    ...
    for irow_a in 0..nrow_a {
        for k in &a_idx2col[a_row2idx[irow_a]..a_row2idx[irow_a + 1]] {
            for jcol_b in &b_idx2col[b_row2idx[*k]..b_row2idx[*k + 1]] {
                if col2flag[*jcol_b] == irow_a || (*jcol_b == irow_a && c_is_square) { continue; }
                let c_ind = c_row2ind[irow_a];
                c_row2ind[irow_a] += 1;
                c_ind2col[c_ind] = *jcol_b;
                col2flag[*jcol_b] = irow_a;
            }
            if b_is_square {
                if col2flag[*k] == irow_a || (*k == irow_a && c_is_square) { continue; }
        ...
        if a_is_square {
            ...
            if b_is_square {
                ...
                let c_ind = c_row2ind[irow_a];
                c_row2ind[irow_a] += 1;
                c_ind2col[c_ind] = irow_a;
                col2flag[irow_a] = irow_a;
            }
        }
    }
    for irow0 in (1..nrow_a).rev() {
        c_row2ind[irow0] = c_row2ind[irow0 - 1];
    }
    c_row2ind[0] = 0;
    (c_row2ind, c_ind2col)
}

pub fn mult_square_matrices(
    m0: &crate::sparse_square::Matrix<f32>,
    m1: &crate::sparse_square::Matrix<f32>) -> crate::sparse_square::Matrix<f32> {
    ...
    let mut idx2val = vec!(0_f32; idx2col.len());
    let mut row2val = vec!(0_f32; num_blk);
    ...
    crate::sparse_square::Matrix::<f32> {
        num_blk,
        row2idx,
        idx2col,
        idx2val,
